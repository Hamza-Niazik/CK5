...
class CollapseText extends FilterBase {
  ...
  public function process($text, $langcode) {
    ...
    if (count($levels)) {
      ...
      $text = \Drupal::service('renderer')->render($holder);
    }
    ...
    if (empty($title)) {
      if ($item['value'][0]['type'] == 'text') {
        ...
        if (preg_match('/(<h\d[^>]*>(.+?)<\/h\d>)/smi', $item['value'][0]['value'], $h_matches)) {
          // Allow nested HTML tags in headings to be carried over in section's
          // title (summary). For example, do not strip <em> or <span> tags.
          $title = $h_matches[2];
        }
